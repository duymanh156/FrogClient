...
import dev.idhammai.api.events.impl.ClickBlockEvent;
import dev.idhammai.api.events.impl.ClientTickEvent;
import dev.idhammai.api.events.impl.PacketEvent;
import dev.idhammai.api.events.impl.RotationEvent;
...
import dev.idhammai.mod.modules.settings.enums.Timing;
...
this.startTime.reset();
    ...
    boolean faceVector(Vec3d directionVec) {
        if (!this.shouldYawStep()) {
            Frog.ROTATION.lookAt(directionVec);
            return true;
        }
        this.sync.reset();
        this.directionVec = directionVec;
        if (Frog.ROTATION.inFov(directionVec, this.fov.getValueFloat())) {
            return true;
        }
    ...
    float getDigSpeed(BlockState state, ItemStack itemStack) {
        ...
        boolean bl = inWeb = this.checkWeb.getValue() && Frog.PLAYER.isInWeb((PlayerEntity)PacketMine.mc.player) && PacketMine.mc.world.getBlockState(this.breakPos).getBlock() == Blocks.COBWEB;
